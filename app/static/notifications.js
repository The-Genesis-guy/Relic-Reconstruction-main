...
(function () {
    ...
    function updateTimersOnPage() {
        const timerElements = document.querySelectorAll('.contest-timer, #contestTimer');
        const now = Date.now() + serverOffset;

        timerElements.forEach(el => {
            const contestId = el.dataset.contestId || (window.SPA_STATE && window.SPA_STATE.contestId);
            if (!contestId) return;

            const contest = activeContests.get(String(contestId));
            if (!contest) return;
            ...
            if (now < startTime) {
                ...
                el.innerHTML = `<span style="color: var(--primary, #3498db);">Starts in: ${formatTime(remaining)}</span>`;
                if (el.id === 'contestTimer') el.textContent = `Starts in: ${formatTime(remaining)}`;
            } else if (now < endTime) {
                const remaining = Math.floor((endTime - now) / 1000);
                el.innerHTML = `<span style="color: var(--success, #10b981);">Ends in: ${formatTime(remaining)}</span>`;
                if (el.id === 'contestTimer') el.textContent = `⏱️ ${formatTime(remaining)}`;
            } else {
                el.innerHTML = `<span style="color: var(--danger, #ef4444);">Contest Ended</span>`;
                if (el.id === 'contestTimer') el.textContent = "Contest Ended";
            }
        });

        // Mirror to statusTimer if it exists (legacy support for problem.html)
        const statusTimer = document.getElementById('statusTimer');
        const mainTimer = document.querySelector('.contest-timer');
        if (statusTimer && mainTimer) {
            statusTimer.textContent = mainTimer.textContent;
        }
    }

    // Global interval for smooth countdown
    setInterval(updateTimersOnPage, 1000);
    ...
    function syncWithServer(data) {
        ...
        if (data.contests) {
            // Synchronize map with server data
            const currentIds = new Set(data.contests.map(c => String(c.id)));

            // Remove contests no longer in the list
            for (const id of activeContests.keys()) {
                if (!currentIds.has(id)) activeContests.delete(id);
            }

            // Update/Add contests
            data.contests.forEach(c => {
                activeContests.set(String(c.id), c);
            });
        }
        updateTimersOnPage();
    }
